...
if [ "$FULL_MODE" = true ] || [ -n "$GEOMETRY" ]; then
    ...
    if [ -s "$FILENAME" ]; then
        ...
        # Не сохраняем снимок на диск — удаляем файл после копирования в буфер.
        rm -f "$FILENAME"
        notify-send -a "$(t "screenshot.notifications.screenshot_app_name")" -t 1500 "Снимок скопирован в буфер обмена"
    fi
    exit 0
fi
